...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        try {
          ...
          if (profile.targetExam && profile.targetExam !== currentExam) {
            setCurrentExam(profile.targetExam);
            localStorage.setItem('cs_mcq_active_exam', profile.targetExam);
          } else if (currentExam && !profile.targetExam) {
            await syncUserData(user.uid, { targetExam: currentExam });
          }
          
          // Minimize Firestore reads: Check local cache first before downloading
          const localAttempts = getQuizAttempts();
          const localBookmarks = getBookmarks();
          const localWrongQs = getWrongQuestions();

          let fbAttempts: QuizAttempt[] = [];
          let fbBookmarks = [];
          let fbWrongQs = [];

          if (localAttempts.length === 0) {
            fbAttempts = await getUserAttempts(user.uid);
            if (fbAttempts.length > 0) saveAllAttempts(fbAttempts);
          }
          if (localBookmarks.length === 0) {
            fbBookmarks = await getUserBookmarks(user.uid);
            if (fbBookmarks.length > 0) saveAllBookmarks(fbBookmarks);
          }
          if (localWrongQs.length === 0) {
            fbWrongQs = await getUserWrongQuestions(user.uid);
  ...
  useEffect(() => {
    if (currentUser && currentExam && userProfile && userProfile.targetExam !== currentExam) {
      syncUserData(currentUser.uid, { targetExam: currentExam }).then((updated) => {
        if (updated) {
          setUserProfile(updated);
        }
      });
    }
  }, [currentExam, currentUser, userProfile?.targetExam]);
  ...
  const triggerBookmarkBadgeUnlock = async () => {
    ...
    if (!existingBadges.some(b => b.id === 'revision_scholar')) {
      ...
      if (currentUser) {
        const updated = await syncUserData(currentUser.uid, {
        ...
        localStorage.setItem('cs_mcq_badges', JSON.stringify(updatedBadges));
      }
    }
  };
  ...
  const handleQuizFinished = async (newAttempt: QuizAttempt) => {
    ...
    // 3. Sync persistence
    if (currentUser) {
      const updatedProfile = await syncUserData(currentUser.uid, {
        points: currentTotalPoints,
        badges: unlockedBadges,
        newAttempts: [newAttempt]
      });
      if (updatedProfile) {
              ...
              <Suspense fallback={
                ...
                {view === 'home' && (
                  <div className="space-y-4 h-full">
                    {/* Miniature sign-in reminder if not logged in */}
                    {!currentUser && !showAuthScreen && (
                      <div 
                        onClick={() => setShowAuthScreen(true)}
                        className="p-3 bg-indigo-500/10 border border-indigo-500/20 hover:border-indigo-400/40 rounded-xl flex items-center justify-between cursor-pointer transition-all"
                      >
                        <div className="flex items-center gap-2 text-xs">
                          <Icons.UserPlus className="w-4 h-4 text-indigo-400" />
                          <span className="text-slate-700 dark:text-slate-300 font-semibold">Sign In for global cloud syncing</span>
                        </div>
