...
export default function PracticeView({
  ...
  const [mode, setMode] = useState<'mock' | 'custom' | 'pyq'>('mock');
  const [selectedPyq, setSelectedPyq] = useState<string>('');

  // Extract PYQ mocks from question pool
  const availablePyqs = useMemo(() => {
    if (!currentExamConfig) return [];
    const pyqQs = questionPool.filter(q => q.exam === currentExamConfig.id && q.source === 'Mock Upload');
    const uniqueTitles = Array.from(new Set(pyqQs.map(q => q.subtopic)));
    return uniqueTitles.sort();
  }, [questionPool, currentExamConfig]);

  useEffect(() => {
    if (availablePyqs.length > 0 && !availablePyqs.includes(selectedPyq)) {
      setSelectedPyq(availablePyqs[0]);
    }
  }, [availablePyqs, selectedPyq]);
  ...
  const handleLaunch = () => {
    if (mode === 'mock') {
      ...
      // Launch PYQ/Mock exactly (isMockExam = true so it counts in average stats)
      onSelectSubtopic("Entire Syllabus", selectedPyq, "mixed", true, true, undefined);
    } else {
  ...
  return (
    <div className="space-y-4 animate-fade-in text-slate-800 dark:text-slate-100 font-sans">
      ...
      {/* Target Mode Segment selector (Mock Exam Pattern vs Custom Drill vs PYQ) */}
      <div className={`grid grid-cols-3 gap-2 p-1 border rounded-2xl h-11 shrink-0 overflow-x-auto ${theme === 'dark' ? 'bg-[#161A1D]/85 border-white/10' : 'bg-white/45 border-white/40 backdrop-blur-md'}`}>
        <motion.button
          whileTap={{ scale: 0.96 }}
          type="button"
          onClick={() => {
          ...
          className={`rounded-xl font-bold text-[11px] truncate flex items-center justify-center gap-1.5 transition-all cursor-pointer min-w-[120px] ${
            mode === 'mock' 
              ? theme === 'dark'
        ...
        >
          <Icons.FileArchive className="w-3.5 h-3.5 text-amber-500" />
          <span>PYQ / Mocks</span>
        </motion.button>
      </div>
        ...
        {mode === 'pyq' && (
          ...
          >
            <div className="flex items-start gap-2.5 mb-2">
              <div className={`p-2 border rounded-xl shrink-0 ${
                theme === 'dark' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-amber-500/10 border-amber-500/20 text-amber-600'
              }`}>
                <Icons.FileArchive className="w-5 h-5 animate-pulse" />
              </div>
              <div>
                <h4 className={`text-xs font-black uppercase tracking-wide ${theme === 'dark' ? 'text-amber-500' : 'text-amber-600'}`}>
                  PYQ & Uploaded Full Mocks
                </h4>
                <p className={`text-[10.5px] mt-1 leading-relaxed ${theme === 'dark' ? 'text-slate-350' : 'text-[#6C737F]'}`}>
                  Select a previously uploaded full mock exam or Previous Year Question paper. These tests have a fixed set of questions.
                </p>
              </div>
            </div>

            <div className={`border rounded-2xl p-3 shadow-sm space-y-1 ${theme === 'dark' ? 'bg-black/20 border-white/5' : 'bg-white/40 border-slate-200'}`}>
              <label className={`block text-[8.5px] font-mono font-black uppercase tracking-wider ${theme === 'dark' ? 'text-amber-500' : 'text-amber-600'}`}>
              ...
              {availablePyqs.length > 0 ? (
                <select
                  ...
                  className={`w-full border rounded-xl p-2.5 outline-none text-xs cursor-pointer font-semibold ${
                    theme === 'dark'
                      ? 'bg-[#0B0C0E]/50 border-white/5 text-slate-200 focus:border-amber-500'
                      : 'bg-slate-50 border-slate-200 text-slate-800 focus:border-amber-500'
                  }`}
                >
                  {availablePyqs.map((pyq, idx) => (
                    <option key={idx} value={pyq}>{pyq}</option>
                  ))}
                </select>
              ) : (
                <div className={`text-xs p-3 rounded-lg border ${theme === 'dark' ? 'bg-white/5 border-white/10 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
                  No full mocks or PYQs uploaded for this exam yet. Use the Upload tab to add them!
                </div>
              )}
            </div>
            ...
            {availablePyqs.length > 0 && (
              <div className={`grid grid-cols-2 gap-2 border-t pt-3 text-[10px] font-mono ${theme === 'dark' ? 'border-white/5 text-slate-400' : 'border-slate-200 text-[#6C737F]'}`}>
                <div className="flex items-center gap-1.5">
                  <Icons.Layers className={`w-3.5 h-3.5 ${theme === 'dark' ? 'text-amber-500' : 'text-amber-600'}`} />
                  <span>Fixed Qs Match</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <Icons.Timer className="w-3.5 h-3.5 text-rose-500 dark:text-rose-400" />
                  <span>Timer: <strong>{currentExamConfig.rules.timeLimitMinutes} Mins</strong></span>
                </div>
              </div>
            )}
      ...
      <motion.button
        ...
        disabled={mode === 'pyq' && availablePyqs.length === 0}
