...
export function saveCustomQuestions(newQuestions: Question[]): void {
  try {
    ...
    try {
      ...
      console.warn('[Storage] LocalStorage quota exceeded, fallback to last 1000 questions:', err);
      try {
        localStorage.setItem(CUSTOM_QUESTIONS_KEY, JSON.stringify(updatedCustom.slice(-1000)));
      } catch (innerErr) {
